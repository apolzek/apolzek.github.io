...
That is when you roll up your sleeves and start doing reverse engineering of the product or platform.

In practice, you start somewhere. You map connections, figure out which server talks to which one, where the databases are, how read and write flows work. What runs on Kubernetes, what is still on VMware. A hybrid environment, with cloud mixed in. Even with sophisticated tools, the environment is complex. At some point, you have to go all the way down to the network packet level and run tcpdump without mercy.
...
Nothing I am proposing here is new. All of this already exists and works well. If there is any merit here, it is probably just about connecting the dots. “Connect the dots,” as Steve Jobs once said.
...
6. If the change is approved, execute the GMUD or RFC. Do it with a clear plan: post-execution tests, a rollback plan, documented monitoring, and properly tuned alerts. No production heroics.
...
8. Update the official Mermaid diagram with all the changes that came out of the Excalidraw session. Add context and notify the team and stakeholders about the change.
