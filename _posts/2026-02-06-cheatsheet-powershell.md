...
Export the list of installed features on the source server:

```powershell
(Get-WindowsFeature | Where-Object {$_.Installed -eq $true}).Name -join "," | Out-File C:\features-to-install.txt
```

Then install the same features on the target server:
...
### Check and Configure WinRM (Windows Remote Management)
