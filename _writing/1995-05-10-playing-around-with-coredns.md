...
Add the VM's IP to `/etc/resolv.conf`
```text
nameserver 192.168.100.218 # VM with Ubuntu Server and CoreDNS
```

> On modern distros `/etc/resolv.conf` is usually managed by `systemd-resolved` or `NetworkManager` and will be overwritten on reboot. To make the change permanent, configure it through the active resolver (`resolvectl dns <iface> 192.168.100.218`) or point `dig` at the VM directly as shown below.
