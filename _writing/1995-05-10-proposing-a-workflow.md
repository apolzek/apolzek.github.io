...
4. With the flow documented, it is time to work on top of it. **Create a page for ADRs** (*Architecture Decision Records*). For each proposed change, document the change itself, the context, the options (P1, P2, P3), what is expected from each, and whether it was accepted or not. If it was accepted, state which one. Depending on the company, this will turn into a GMUD (a Brazilian-Portuguese acronym for change management ticket) or an RFC, the formal change document.
...
8. **Update the official Mermaid diagram** with all the changes that came out of the Excalidraw session. Add context and notify the team and stakeholders about the change after it is rolled out.
...
flowchart LR
    ...
    linkStyle 2 stroke:#3b82f6,stroke-width:3px,stroke-dasharray: 5 5
```
